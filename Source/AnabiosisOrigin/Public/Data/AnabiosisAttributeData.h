...
struct FAnabiosisAttributeData : public FTableRowBase
{
	...
	// --- 基础属性 ---
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "AttackComponent|Attributes|Primary", meta = (ClampMin = "1")) // 细化分类
	...
	// --- 战斗属性 (通常是初始值或基础值) ---
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "AttackComponent|Attributes|Combat", meta = (ClampMin = "0")) // 细化分类
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "AttackComponent|Attributes|Combat", meta = (ClampMin = "0.0", ClampMax = "1.0")) // 细化分类
	float CriticalChance = 0.05f; // 暴击率

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "AttackComponent|Attributes|Combat", meta = (ClampMin = "1.0")) // 细化分类
	float CriticalMultiplier = 1.5f; // 暴击倍率

	// --- 衍生属性系数 (可选，如果属性计算在 GE 中处理，则可能不需要) ---
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "AttackComponent|Attributes|Coefficients", meta = (ClampMin = "0")) // 细化分类
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "AttackComponent|Attributes|Growth", meta = (ClampMin = "1.0")) // 细化分类
    ...
    // --- 武器 ---
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "AttackComponent|Equipment|Weapon", meta = (ToolTip = "武器属性数据表中的行名")) // 细化分类
    FName WeaponDataRowName = NAME_None; // 武器数据行名

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "AttackComponent|Equipment|Weapon", meta = (ToolTip = "武器附加到骨骼上的插槽名称")) // 细化分类
    FName WeaponAttachSocketName = "hand_rSocket"; // 默认插槽

	// --- 动画 ---
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "AttackComponent|Animation|Reactions") // 细化分类
